...
## Phase 5: Orchestration — Planner + Router + Combiner ✅

**Goal**: Wire all agents into the LangGraph graph with correct memory and streaming.

### LangGraph Graph (Correct Implementation) `[CHANGED]`
```python
from langgraph.graph import StateGraph, END
from langgraph.checkpoint.memory import MemorySaver

builder = StateGraph(GraphState)
builder.add_node("planner", planner_node)    # LLM → PlannerOutput
builder.add_node("router", router_node)      # Pure Python — runs agents
builder.add_node("combiner", combiner_node)  # LLM → final answer
...
# MemorySaver stores full GraphState per (thread_id, checkpoint_id)
checkpointer = MemorySaver()
graph = builder.compile(checkpointer=checkpointer)
```

Note: No conditional edges, no Send(). Three nodes, two edges. The router handles all branching in Python.

### Planner Structured Output
```python
class PlannerOutput(BaseModel):
    agents: list[Literal["rag", "sql", "doc_parser"]]
    queries: dict[str, str]   # {"rag": "rewritten query for RAG", "sql": "..."}
    parallel: bool
    reasoning: str            # stored in metadata, not shown to user

# Usage:
planner_llm = ChatOpenAI(model="gpt-4o-mini").with_structured_output(PlannerOutput)
```

### Planner System Prompt Design
```
You are a routing planner for an HR AI assistant.
Analyze the user's query and decide which agents to invoke.

Available agents:
- "rag": searches HR policy documents
- "sql": queries the employee/HR database  
- "doc_parser": extracts info from uploaded files

Rules:
1. If no file is uploaded, never include "doc_parser"
2. For policy questions → ["rag"]
3. For data/numbers/employee lookup → ["sql"]  
4. For uploaded file analysis → ["doc_parser"] first, then other agents if needed
5. For complex cross-agent tasks → order agents by dependency (producer first)
6. Set parallel=true ONLY if agents are independent (no output dependency)

Return a PlannerOutput JSON. No explanations.
```

### Combiner Conflict Resolution (Concrete) `[CHANGED]`
```python
async def combiner_node(state: GraphState) -> GraphState:
    results = state["agent_results"]
    successful = [r for r in results if r.success]
    failed = [r for r in results if not r.success]

    if not successful:
        return {**state, "final_answer": "I was unable to find information. Please rephrase."}

    # Combiner prompt explicitly includes conflict detection instruction
    combiner_prompt = build_combiner_prompt(successful, failed, state["query"])
    response = await combiner_llm.ainvoke(combiner_prompt)
...
├── __init__.py
├── planner.py      # planner_node — LLM → PlannerOutput (structured output)
├── router.py       # router_node — pure Python, asyncio.gather
├── combiner.py     # combiner_node — LLM → final answer with conflict policy
├── graph.py        # StateGraph assembly + MemorySaver
└── agent_registry.py  # {name: BaseAgent} lookup dict, built by factory
```

### Test
```bash
docker compose exec backend pytest tests/test_orchestration.py -v
# Tests: planner output parsing, router parallel vs sequential,
#        combiner with 1/2/3 results, combiner with partial failure,
#        full graph smoke test (mocked agents)
```
